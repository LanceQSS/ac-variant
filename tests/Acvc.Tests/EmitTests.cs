...
public class EmitTests : IDisposable
{
    ...
    private static string UiJsonText(string carName)
        => "{\n\t\"name\": \"Fake " + carName + "\",\n\t\"brand\": \"Kunos\",\n\t\"class\": \"street\",\n" +
           "\t\"specs\":\n\t{\n\t\t\"bhp\": \"100bhp\",\n\t\t\"torque\": \"100Nm\",\n\t\t\"weight\": \"900kg\",\n" +
           "\t\t\"pwratio\": \"9.00kg/hp\"\n\t},\n" +
           "\t\"torqueCurve\": [[\"0\",\"0\"],[\"1000\",\"100\"]],\n" +
           "\t\"powerCurve\": [[\"0\",\"0\"],[\"1000\",\"14\"]]\n}";
    ...
    public void Variant_structure_is_complete_and_correct(string carName)
    {
        ...
        // Readme: source car, spec text, tool version (whatever the assembly says).
        ...
        Assert.Contains($"Generated by acvc {version}", readme);
    ...
    [SkippableFact]
    public void Emit_applies_the_ui_patch_when_provided()
    {
        var source = CreateSourceCar("abarth500");
        var files = Acvc.Core.Acd.AcdUnpacker.Load(source).Files;
        var models = CarModelSet.FromFiles(files);
        var validation = TunePipeline.Apply(new TunePlan
        {
            SourceCar = "abarth500",
            TuneName = "test_tune",
            MassTotal = 1234,
        ...
        Assert.False(validation.HasFailures);
        var patch = Acvc.Core.UiMeta.UiCarPatcher.BuildPatch(models);

        var result = VariantEmitter.Emit(source, "abarth500_test_tune", models.MergedInto(files),
            Options() with { UiPatch = patch });

        var ui = File.ReadAllText(Path.Combine(result.VariantPath, "ui", "ui_car.json"));
        Assert.Contains("Fake abarth500 — test_tune", ui);   // name suffix still applied
        Assert.Contains($"\"{patch.Bhp}\"", ui);              // regenerated specs in place
        Assert.Contains($"\"{patch.Weight}\"", ui);
        Assert.DoesNotContain("\"100bhp\"", ui);
        Assert.DoesNotContain("[\"1000\",\"100\"]", ui);      // old curves replaced
    }
